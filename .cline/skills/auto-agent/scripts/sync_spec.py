#!/usr/bin/env python3
"""
Spec Sync — splits Agent_SPEC.md into chapter files under auto-agent/references/.
...
def sync(force: bool = False):
    skill_dir = Path(__file__).parent.parent          # auto-agent/
    ...
    if not agent_spec.exists():
        print(f"ERROR: {agent_spec} not found"); sys.exit(1)

    # Hash check
    current_hash = hashlib.sha256(agent_spec.read_bytes()).hexdigest()
    if not force and hash_file.exists() and hash_file.read_text().strip() == current_hash:
        print("specs up-to-date"); return

    content = agent_spec.read_text(encoding='utf-8')
